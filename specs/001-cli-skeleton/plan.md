...
**Testing**: `go test ./...` с coverage (unit/contract-тесты CLI через `cobra` Execute + захват stdout/stderr); `make test` / `make build`; покрытие MUST NOT падать (constitution IX)
...
| G9 | Test-Driven Development & Coverage | PASS | unit-тесты CLI до/вместе с кодом; `make test`; coverage gate — follow-up усиления Makefile |
